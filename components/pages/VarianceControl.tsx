...
import { 
  ...
  BarChart3, Settings, Search, Warehouse, Factory, 
  Truck, ArrowDown, ClipboardList, TrendingUp,
  Pill, Building2, HardHat, ShieldCheck, Network,
  Banknote, FileCheck, ShoppingCart
} from 'lucide-react';
import { Button } from '../Button';

interface VarianceControlProps {
  onOpenDemo: () => void;
  onContact: () => void;
}

export const VarianceControl: React.FC<VarianceControlProps> = ({ onOpenDemo, onContact }) => {
    ...
    <div className="bg-white min-h-screen">
      ...
      <section className="relative bg-white pt-32 pb-20 lg:pt-48 lg:pb-32 overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10 text-center">
            <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-white border border-slate-200 shadow-sm mb-8">
              ...
              <span className="text-xs font-semibold text-slate-600 uppercase tracking-widest">Variance & Loss Prevention</span>
            </div>
            
            <h1 className="text-4xl md:text-6xl font-extrabold text-slate-900 leading-tight tracking-tight mb-8 max-w-4xl mx-auto">
              Detect Material Losses <br />
            ...
            <p className="text-xl text-slate-600 mb-10 leading-relaxed max-w-2xl mx-auto">
              Identify, track, and control quantity and weight variances in material movement with real-time visibility and audit-ready records.
            </p>
            ...
            <div className="bg-white rounded-3xl p-8 lg:p-10 border border-slate-200 shadow-sm relative overflow-hidden group hover:shadow-xl transition-all duration-300">
              ...
              <div className="relative z-10">
                <h2 className="text-2xl font-bold text-slate-900 mb-6 flex items-center gap-3">
                  <div className="bg-red-50 p-2 rounded-lg text-red-500 border border-red-100"><AlertTriangle className="w-6 h-6" /></div>
                  The Challenge
                </h2>
                <p className="text-slate-600 mb-6">
                  Material movement variances often go unnoticed due to:
                </p>
                <ul className="space-y-4">
                  {[
                    "Mismatch between physical quantity and documented records",
                    "Weight discrepancies during dispatch or receipt",
                    "Delayed detection of losses or pilferage",
                    "Manual reconciliation between gate, stores, and finance",
                    "Audit observations related to inventory and material control"
                  ].map((item, i) => (
            ...
            <div className="bg-white rounded-3xl p-8 lg:p-10 shadow-2xl shadow-red-900/5 relative overflow-hidden border-2 border-movilance-red group">
               ...
               <div className="relative z-10">
                 <h2 className="text-2xl font-bold text-movilance-navy mb-6 flex items-center gap-3">
                    <div className="bg-movilance-red p-2 rounded-lg text-white shadow-lg"><Scale className="w-6 h-6" /></div>
                    The Movilance Solution
                 </h2>
                 <p className="text-lg text-slate-600 leading-relaxed mb-6">
                   Movilance Variance Control continuously monitors material movement by comparing physical checks, weighbridge data, and document values to detect discrepancies early and prevent financial losses.
                 </p>
                 <div className="bg-slate-50 p-6 rounded-xl border border-slate-200">
                   <p className="text-slate-700 font-medium italic border-l-4 border-movilance-red pl-4">
                     "We reduced material shortages by 90% by flagging weight discrepancies instantly at the exit gate."
                   </p>
                 </div>
              </div>
            </div>
      ...
      <section className="py-24 bg-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center max-w-3xl mx-auto mb-16">
            ...
            <p className="text-slate-600 text-lg">Precision control over every gram and unit.</p>
          </div>
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
            {[
              {
                icon: ClipboardList,
                title: "Quantity Variance Tracking",
                features: ["Compare PO/Invoice vs Physical", "Auto-calculate shortages/excess", "Track by material or vendor"]
              },
              {
                icon: Scale,
                title: "Weight Variance Monitoring",
                features: ["Tare & Gross weight capture", "Manual & Weighbridge workflows", "Identify abnormal differences"]
              },
              {
                icon: Settings,
                title: "Tolerance & Exception Rules",
                features: ["Configure acceptable limits", "Flag violating transactions", "Enable early corrective action"]
              },
              {
                icon: TrendingUp,
                title: "Variance Reports & Analysis",
                features: ["View variance trends over time", "Vendor and route level analysis", "Export reports for audit"]
              },
              {
                icon: Network,
                title: "Seamless Integration",
                features: ["Integration with Material modules", "Connect with ERP systems", "Bi-directional data flow"]
              },
              {
                icon: ShieldCheck,
                title: "Enterprise Security",
                features: ["Role based access control", "Complete transaction audit trail", "Secure data storage"]
              }
            ].map((cap, idx) => (
      ...
      <section className="py-24 bg-slate-50 relative overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
          <div className="text-center mb-20">
             ...
             <p className="text-slate-600 mt-4 text-lg">Stop losses in five steps.</p>
          </div>
          ...
          <div className="grid md:grid-cols-5 gap-8 relative">
             ...
             {[
               { step: "01", title: "Capture", desc: "Log material docs (PO/Invoice/Challan)." },
               { step: "02", title: "Verify", desc: "Check quantity or weight at gate." },
               { step: "03", title: "Detect", desc: "System auto-compares with documents." },
               { step: "04", title: "Review", desc: "Exceptions flagged for action." },
               { step: "05", title: "Audit", desc: "Data stored in variance audit trail." }
             ].map((item, idx) => (
          ...
          <div className="grid md:grid-cols-2 gap-16">
            <div>
              ...
              <div className="space-y-6">
                {[
                  "Early detection of material discrepancies",
                  "Reduced dependency on manual reconciliation",
                  "Better coordination between gate, stores, and finance"
                ].map((b, i) => (
            ...
            <div>
              <h3 className="text-2xl font-bold text-movilance-navy mb-8 border-l-4 border-movilance-red pl-4">Financial Control Benefits</h3>
              <div className="space-y-6">
                {[
                  "Prevention of material losses and pilferage",
                  "Improved overall inventory accuracy",
                  "Stronger financial and audit control"
                ].map((b, i) => (
                   <div key={i} className="flex gap-4 p-4 rounded-xl bg-slate-50 border border-slate-100">
                     <div className="bg-white rounded-full p-1 h-fit text-movilance-red shadow-sm border border-red-100"><BarChart3 className="w-5 h-5" /></div>
                     <span className="text-slate-700 font-medium">{b}</span>
                   </div>
                ))}
              </div>
      ...
      <section className="py-20 bg-slate-50">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
           <div className="text-center mb-12">
              <h2 className="text-2xl font-bold text-movilance-navy">Useful For Teams</h2>
              <p className="text-slate-600 mt-2">Empowering stakeholders with data.</p>
           </div>
           
           <div className="grid md:grid-cols-4 gap-6">
              {[
                { icon: Warehouse, role: "Stores & Warehouse", desc: "Monitoring inbound and outbound materials" },
                { icon: Banknote, role: "Finance", desc: "Reconciliation and loss prevention" },
                { icon: FileCheck, role: "Audit & Compliance", desc: "Reviewing variance records" },
                { icon: Truck, role: "Supply Chain", desc: "Vendor and logistics performance tracking" },
              ].map((team, idx) => (
                 <div key={idx} className="bg-white p-6 rounded-xl border border-slate-200 text-center shadow-sm hover:shadow-md transition-all">
                    <div className="w-12 h-12 bg-red-50 text-movilance-red rounded-full flex items-center justify-center mx-auto mb-4">
                       <team.icon className="w-6 h-6" />
                    </div>
                    <h3 className="font-bold text-slate-800 mb-2">{team.role}</h3>
      ...
      <section className="py-20 bg-white border-t border-slate-100">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
           <h2 className="text-2xl font-bold text-movilance-navy mb-10">Ideal For Industries</h2>
           
           <div className="flex flex-wrap justify-center gap-4">
              {[
                { icon: Factory, label: "Manufacturing" },
                { icon: HardHat, label: "Metals & Cement" },
                { icon: ShoppingCart, label: "FMCG" },
                { icon: Pill, label: "Pharma" },
                { icon: Building2, label: "Infrastructure" },
              ].map((ind, idx) => (
                 <div key={idx} className="flex items-center gap-3 px-8 py-4 bg-slate-50 rounded-2xl border border-slate-200 text-slate-700 font-semibold shadow-sm hover:shadow-md hover:border-movilance-red/30 transition-all duration-300 group">
