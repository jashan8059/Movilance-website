...
import { 
  Key, Lock, Search, ShieldCheck, FileCheck, 
  Building2, Factory, Stethoscope, GraduationCap, 
  ArrowDown, Timer, UserCheck, AlertTriangle
} from 'lucide-react';
import { Button } from '../Button';

interface LockersAndKeysProps {
  onOpenDemo: () => void;
  onContact: () => void;
}

export const LockersAndKeys: React.FC<LockersAndKeysProps> = ({ onOpenDemo, onContact }) => {
      ...
      <section className="relative bg-white pt-32 pb-20 lg:pt-48 lg:pb-32 overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10 text-center">
            <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-white border border-slate-200 shadow-sm mb-8">
              ...
              <span className="text-xs font-semibold text-slate-600 uppercase tracking-widest">Key & Locker Management</span>
            </div>
            
            <h1 className="text-4xl md:text-6xl font-extrabold text-slate-900 leading-tight tracking-tight mb-8 max-w-4xl mx-auto">
              Every Key Accounted For. <br />
            ...
            <p className="text-xl text-slate-600 mb-10 leading-relaxed max-w-2xl mx-auto">
              Digitally manage the issuance, return, and custody of physical keys, lockers, and access tokens with complete accountability.
            </p>
            ...
            <div className="bg-white rounded-3xl p-8 lg:p-10 border border-slate-200 shadow-sm relative overflow-hidden group hover:shadow-xl transition-all duration-300">
              ...
              <div className="relative z-10">
                <h2 className="text-2xl font-bold text-slate-900 mb-6 flex items-center gap-3">
                  <div className="bg-red-50 p-2 rounded-lg text-red-500 border border-red-100"><Lock className="w-6 h-6" /></div>
                  The Challenge
                </h2>
                <p className="text-slate-600 mb-6">
                  Physical keys and lockers are often managed through manual registers, leading to:
                </p>
                <ul className="space-y-4">
                  {[
                    "Lost or unreturned keys due to lack of tracking",
                    "No real-time visibility on who currently holds a key",
                    "Unauthorized access to critical areas via duplicate keys",
                    "Difficulty tracing history during security investigations"
                  ].map((item, i) => (
            ...
            <div className="bg-white rounded-3xl p-8 lg:p-10 shadow-2xl shadow-red-900/5 relative overflow-hidden border-2 border-movilance-red group">
               ...
               <div className="relative z-10">
                 <h2 className="text-2xl font-bold text-movilance-navy mb-6 flex items-center gap-3">
                    <div className="bg-movilance-red p-2 rounded-lg text-white shadow-lg"><Key className="w-6 h-6" /></div>
                    The Movilance Solution
                 </h2>
                 <p className="text-lg text-slate-600 leading-relaxed mb-6">
                   Movilance Lockers & Keys provides a structured digital workflow to issue, track, and audit physical keys and lockers, ensuring secure access control and operational discipline.
                 </p>
                 <div className="bg-slate-50 p-6 rounded-xl border border-slate-200">
                   <p className="text-slate-700 font-medium italic border-l-4 border-movilance-red pl-4">
                     "We now have instant visibility on every master key in our facility, eliminating unauthorized access completely."
                   </p>
                 </div>
              </div>
            </div>
      ...
      <section className="py-24 bg-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center max-w-3xl mx-auto mb-16">
            ...
            <p className="text-slate-600 text-lg">Digital custody for your physical security assets.</p>
          </div>
            ...
            {[
              {
                ...
                features: ["Issue keys/lockers from security desk", "Capture user & department details", "Record purpose of access"]
              },
              {
                icon: Timer,
                title: "Return Tracking",
                features: ["Real-time return status", "Expected return time logging", "Reduce manual follow-ups"]
              },
              {
                icon: AlertTriangle,
                title: "Overdue Alerts",
                features: ["Notifications for delayed returns", "Escalation to supervisors", "Identify missing keys instantly"]
              },
              {
                icon: UserCheck,
                title: "Access Accountability",
                features: ["Complete custody history", "Prevent unauthorized handovers", "Digital signature capture"]
              },
              {
                icon: Search,
                title: "Search & Audit Logs",
                features: ["Search by key number or user", "Exportable movement reports", "Detailed timestamp logs"]
              },
              {
                icon: ShieldCheck,
                title: "Master Key Control",
                features: ["Restricted access for master keys", "Multi-level approval for sensitive keys", "High-security audit trail"]
              }
            ].map((cap, idx) => (
      ...
      <section className="py-24 bg-slate-50 relative overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
          <div className="text-center mb-20">
             ...
             <p className="text-slate-600 mt-4 text-lg">Simple steps to secure physical access.</p>
          </div>
          ...
          <div className="grid md:grid-cols-5 gap-8 relative">
             ...
             {[
               { step: "01", title: "Request", desc: "User requests access to key/locker." },
               { step: "02", title: "Issue", desc: "Security logs issuance digitally." },
               { step: "03", title: "Usage", desc: "Access granted for approved duration." },
               { step: "04", title: "Return", desc: "Item returned; transaction closed." },
               { step: "05", title: "Audit", desc: "Record saved for permanent audit." }
             ].map((item, idx) => (
          ...
          <div className="grid md:grid-cols-2 gap-16">
            <div>
              ...
              <div className="space-y-6">
                {[
                  "Faster key issuance and return process",
                  "Elimination of manual registers and paperwork",
                  "Clear visibility of real-time key/locker status"
                ].map((b, i) => (
                   <div key={i} className="flex gap-4 p-4 rounded-xl bg-slate-50 border border-slate-100">
                     <div className="bg-white rounded-full p-1 h-fit text-emerald-600 shadow-sm border border-emerald-100"><FileCheck className="w-5 h-5" /></div>
            ...
            <div>
              ...
              <div className="space-y-6">
                {[
                  "Prevention of key misuse, loss, or unauthorized duplication",
                  "Strengthened physical access control measures",
                  "100% audit-ready key movement records"
                ].map((b, i) => (
                   <div key={i} className="flex gap-4 p-4 rounded-xl bg-slate-50 border border-slate-100">
